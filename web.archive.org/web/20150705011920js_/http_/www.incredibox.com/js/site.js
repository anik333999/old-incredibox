...
function showabp(){
	abpShow = true;
	$("#bbox").addClass("show-abp");
	$("#bbox .popup#box-abp .popup-body").html('<div id="abp"><div id="abp-bck"></div><div id="abp-popup">Oops! Your browser is using the ABP (Ad Block Plus) plugin, you can not access to incredibox.com with the ABP plugin. To play Incredibox, please disable ABP in you browser settings or authorize incredibox.com in the ABP filters (<a href="https://web.archive.org/web/20150705011920/https://adblockplus.org/en/faq_basics#disable" target="_blank">see how to do</a>).<img src="'+domain+'img/abp.jpg" width="390" height="110" />Yes, we know, ads can be annoying, but we want to keep Incredibox free, this ads only cover our servers and maintenance costs.<br><br><span class="info">Don\'t worry, you will never see ads banners about violence, swearing, nudity, etc. You can disable APB safely.</span></div></div>');
	saveGA("www","popup-abp");
}
function hideabp(){
	if(abpShow){
		$("#bbox").removeClass("show-abp");
		$("#bbox .popup#box-abp .popup-body").html('');
		abpShow = false;
	}
}
...
function initHome(){
	// -----
	$(".home #select a.game#appv1").hover(overBtGame, outBtGame);
	$(".home #select a.game#appv2").hover(overBtGame, outBtGame);
	$(".home #select a.game#appv3").hover(overBtGame, outBtGame);
	$(".home #select a.game#appv4").hover(overBtGame, outBtGame);
	// -----
...
function overBtGame(){
	$(".bck", this).addClass("bckhover");
	$(".p1", this).addClass("p1hover");
	TweenMax.to($(".title", this), 0.2,{opacity:0, marginTop:"5px", ease:Quint.easeOut, overwrite:true});
	TweenMax.to($(".year", this), 0.2,{opacity:0, marginTop:"32px", ease:Quint.easeOut, overwrite:true});
	TweenMax.to($(".play", this), 0.2,{opacity:1, marginTop:"0px", ease:Quint.easeOut, overwrite:true, delay:0.1});
}
function outBtGame(){
	$(".bck", this).removeClass("bckhover");
	$(".p1", this).removeClass("p1hover");
	TweenMax.to($(".title", this), 0.2,{opacity:1, marginTop:"0px", ease:Quint.easeOut, overwrite:true, delay:0.1});
	TweenMax.to($(".year", this), 0.2,{opacity:1, marginTop:"27px", ease:Quint.easeOut, overwrite:true, delay:0.1});
	TweenMax.to($(".play", this), 0.2,{opacity:0, marginTop:"5px", ease:Quint.easeOut, overwrite:true});
}
function bestClickNavPoint(){
	...
	if(num!=bestPhrase){
		...
		saveGA("home","nav-point-punchline");
	}
}
function bestClickNavLeft(){
	...
	saveGA("home","nav-left-punchline");
}
function bestClickNavRight(){
	...
	currentP.removeClass("light");
	nextP.addClass("light");
	saveGA("home","nav-right-punchline");
}
...
function overBtHome(){
	...
	TweenMax.to(bt1Picto, 0.2,{top:"25px", opacity:.8, ease:Back.easeOut, overwrite:true});
...
function outBtHome(){
	...
	TweenMax.to(bt1Picto, 0.2,{top:"7px", opacity:.6, ease:Quint.easeOut, delay:0.1, overwrite:true});
...
function shareCompo(_compoURL, _social){
	...
	switch(_social){
		case "facebook":
			url = "https://web.archive.org/web/20150705011920/http://www.facebook.com/sharer/sharer.php?u="+encodeURIComponent(domain+"/mix/"+_compoURL);
		break;
		case "twitter":
			url = "https://web.archive.org/web/20150705011920/http://twitter.com/share?url="+encodeURIComponent(domain+"mix/"+_compoURL)+"&text="+encodeURIComponent("Be cool, vote for my mix on Incredibox! #INCREDIBOX");
		break;
		default:
		break;
	}
...
function overFooterFollow(){
	...
	TweenMax.to(bulle, .2,{alpha:1, marginLeft:"0px", ease:Quint.easeOut, overwrite:true});
}
function outFooterFollow(){
	var bulle = $(".inbox#bulle", this.parent);
	TweenMax.to(bulle, .2,{alpha:0, marginLeft:"10px", ease:Quint.easeOut});
}
...
function openPopup(id, param, close){
	...
	if(!popupOpen){
		...
		TweenMax.to($bfond, 0.3,{opacity:.9, ease:Linear.easeNone});
		...
		switch(id){
			...
			case "box-news":
				...
				saveGA("www","popup-newsletter");
			break;
			case "box-video":
				openVideo(param);
				funcOnClosePopup = closePopupVideo;
				saveGA("goodies","popup-video");
			break;
			case "box-captchme":
				funcOnClosePopup = nada;
				rapportTexte("", "alert-captchme", "", "");
				saveGA("download","popup-captchme");
			break;
...
function openVideo(videoURL){
	var scrVideo = '//web.archive.org/web/20150705011920/http://www.youtube.com/embed/'+videoURL+'?autoplay=1';
...
function callBackSaveDownload(msg, params){ /*alert(msg);*/ }
...
/*
     FILE ARCHIVED ON 01:19:20 Jul 05, 2015 AND RETRIEVED FROM THE
     INTERNET ARCHIVE ON 14:09:46 Feb 22, 2024.
...
playback timings (ms):
  ...
  cdx.remote: 0.135
  esindex: 0.014
  LoadShardBlock: 127.281 (6)
  PetaboxLoader3.datanode: 113.457 (7)
  load_resource: 88.32
  PetaboxLoader3.resolve: 72.931
*/